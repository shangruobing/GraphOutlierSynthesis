...
可以是基于顶点的采样[16]，基于边的采样 [17] ，遍历抽样（traversal-based sampling）[18]，其他先进的方法，如Metropolis-Hastings sampling[19]

### Data Mixup

构造虚拟样本，称为 Mixup[25]，它直接插值训练样本。通过这种方式，混合方法通过结合特征向量的线性插值应该导致相关目标的线性插值来扩展训练分布。类似地，Manifold mixup[26] 对从两个训练样本中学习到的中间表示进行混合。
...
\mathcal{L}_D
```
分布内数据和筛选后的能量集合构建形成新的数据集，图监督学习训练时产生的误差为：
```math
\mathcal{L}_{\text {sup }}=\mathbb{E}_{\left(\mathbf{x}, \mathcal{G}_{\mathbf{x}}, y\right) \sim \mathcal{D}_{\text {in }}}\left(-\log p\left(y \mid \mathbf{x}, \mathcal{G}_{\mathbf{x}}\right)\right)=\sum_{i \in \mathcal{I}_s}\left(-h_\theta\left(\mathbf{x}_i, \mathcal{G}_{\mathbf{x}_i}\right)_{\left[y_i\right]}+\log \sum_{c=1}^C e^{h_\theta\left(\mathbf{x}_i, \mathcal{G}_{\mathbf{x}_i}\right)_{[c]}}\right)
```
整个方法的总误差为
```math
\mathcal{L} = \mathcal{L}_{\text {sup }}+ \mathcal{L}_D
```
其中我们采用的生成点能量传播的
